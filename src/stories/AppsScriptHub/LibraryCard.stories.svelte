<script module>
	...
	const { Story } = defineMeta({
		...
		parameters: {
			docs: {
				description: {
					component: 'GASライブラリの情報を表示するカードコンポーネント。検索結果やライブラリ一覧ページで使用されます。'
				}
			}
		}
	});
	...
	const longDescriptionLibrary = {
		...
		description: 'このライブラリは非常に長い説明文を持っています。この説明文は複数行にわたって表示される可能性があり、カードレイアウトがどのように対応するかをテストするために使用されます。さらに長い文章を追加して、レイアウトの動作を確認します。',
...
<Story 
	...
	play={async ({ canvasElement }) => {
		...
		if (libraryName.textContent !== 'GasDateFormatter') throw new Error('ライブラリ名が正しく表示されていません');
		if (libraryName.getAttribute('href') !== '/libraries/1') throw new Error('ライブラリリンクが正しくありません');
		
		// 説明文が表示されることを確認
		const description = canvasElement.querySelector('p');
		if (!description) throw new Error('説明文が見つかりません');
		if (!description.textContent?.includes('Moment.jsライクなシンタックス')) throw new Error('説明文が正しく表示されていません');
		...
		if (authorLink.getAttribute('href') !== '/users/user-name') throw new Error('作成者リンクが正しくありません');
...
<Story 
	...
	play={async ({ canvasElement }) => {
		...
		const libraryName = canvasElement.querySelector('h2 a');
		if (!libraryName) throw new Error('ライブラリ名が見つかりません');
		if (libraryName.textContent !== 'SuperLongLibraryNameThatMightWrapToMultipleLines') throw new Error('長いライブラリ名が正しく表示されていません');
		
		// 長い説明文が表示されることを確認
		const description = canvasElement.querySelector('p');
		if (!description) throw new Error('説明文が見つかりません');
		if (!description.textContent?.includes('このライブラリは非常に長い説明文')) throw new Error('長い説明文が正しく表示されていません');
...
<Story 
	...
	play={async ({ canvasElement }) => {
		...
		const libraryName = canvasElement.querySelector('h2 a');
		if (!libraryName) throw new Error('ライブラリ名が見つかりません');
		if (libraryName.textContent !== 'SimpleLib') throw new Error('ライブラリ名が正しく表示されていません');
		
		const description = canvasElement.querySelector('p');
		if (!description) throw new Error('説明文が見つかりません');
		if (description.textContent !== 'シンプルなライブラリ') throw new Error('説明文が正しく表示されていません');
...
<Story 
	...
	play={async ({ canvasElement, step }) => {
		await step('ライブラリ名のリンク動作確認', async () => {
			...
			if (libraryNameLink.textContent !== 'GasDateFormatter') throw new Error('ライブラリ名が正しく表示されていません');
			if (libraryNameLink.getAttribute('href') !== '/libraries/1') throw new Error('リンク先が正しくありません');
		});
		...
		await step('作成者のリンク動作確認', async () => {
			...
			const authorLink = Array.from(authorLinks).find(link => link.textContent === 'user-name');
			if (!authorLink) throw new Error('作成者リンクが見つかりません');
			if (authorLink.getAttribute('href') !== '/users/user-name') throw new Error('作成者リンク先が正しくありません');
		});
		
		await step('カード要素の存在確認', async () => {
			const cardElement = canvasElement.querySelector('div');
			if (!cardElement) throw new Error('カード要素が見つかりません');
			if (!cardElement.classList.contains('border') || !cardElement.classList.contains('rounded-lg')) {
